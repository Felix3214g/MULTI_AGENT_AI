...
class NeoCortex:
    ...
    def __init__(self, model_name: str = "deepseek-ai/DeepSeek-V3", temperature: float = 0.2):
        ...
        self.together_client = Together(api_key="3405ce428ec69a2cf4c9f128064e9bc367b630fafc5968d2eff6bbdfaa462067")
        
        # Keep the old API credentials for backward compatibility with any code that might reference them
        self.api_url = "https://openrouter.ai/api/v1/chat/completions"
        self.api_key = "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"
    ...
    def _generate_response(self, prompt: str, emit_thoughts=False, system_prompt=None, task_id=None) -> str:
        """Generate a response using the Together API with retry logic for rate limits."""
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Using Together API client instead of direct API calls
                messages = [
                    {"role": "system", "content": system_prompt},
                    {"role": "user", "content": prompt}
                ]
                
                # Emit thoughts for debugging if requested
                if emit_thoughts and self.thought_generated:
                    self.thought_generated.emit(f"Generating response using {self.model_name}...")
                ...
                if response and response.choices:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        if emit_thoughts and self.thought_generated:
                            self.thought_generated.emit(f"Retrying in {delay} seconds...")
                        time.sleep(delay)
                    else:
                        return f"Error: No response received from the model after {max_retries} attempts."
                        
            except Exception as e:
                if attempt < max_retries - 1:
                    delay = base_delay * (2 ** attempt)
                    if emit_thoughts and self.thought_generated:
                        self.thought_generated.emit(f"Error: {str(e)}. Retrying in {delay} seconds...")
                    time.sleep(delay)
                else:
...
class CreativeEnhancer:
    """Module to enhance the creative capabilities of the multi-agent system."""
    
    def __init__(self, neocortex):
        self.neocortex = neocortex
        self.domain_knowledge = {
            "science": ["analogical reasoning", "first principles thinking", "constraint removal", "cross-disciplinary application"],
            "engineering": ["biomimicry", "reverse engineering", "modular design", "extreme parameter variation"],
            "business": ["blue ocean strategy", "lateral thinking", "job-to-be-done", "disruptive innovation"],
            "arts": ["juxtaposition", "metaphorical thinking", "defamiliarization", "conceptual blending"],
            "general": ["inversion", "abstraction", "random stimulus", "provocative operation", "pattern breaking"]
        }
        self.previous_approaches = set()
        
    def enhance_creativity_prompt(self, query: str, previous_solution: Optional[str] = None,
                                criticism: Optional[str] = None) -> str:
        """Generate an enhanced system prompt for the CreativeAgent based on problem domain and feedback."""
        # Detect the domain of the query
        domains = self._detect_domains(query)
        
        # Get relevant creativity techniques
        techniques = self._get_domain_techniques(domains)
        
        # Generate techniques string
        techniques_prompt = "\n".join([f"- {t.title()}: Apply {t} to generate novel approaches" for t in techniques])
        
        # Include criticism if available
        criticism_prompt = ""
        if criticism:
            criticism_prompt = f"""
            CRITICAL FEEDBACK ON PREVIOUS SOLUTION:
            {criticism}
            
            Use this feedback to enhance your creative approach. Address the identified limitations
            while maintaining the strengths of the previous solution.
            """
            
        # Create enhanced creativity prompt
        enhanced_prompt = f"""You are a creative agent specialized in generating innovative approaches to problems.
Your task is to:
1. Think beyond conventional solutions and constraints
2. Generate multiple diverse and novel approaches
3. Consider interdisciplinary perspectives and analogies
...
DOMAIN-SPECIFIC CREATIVITY TECHNIQUES TO APPLY:
{techniques_prompt}

{criticism_prompt}

For maximum creativity:
1. Start by generating 2-3 times more ideas than needed, then select the most promising ones
2. Deliberately explore alternatives that seem initially counterintuitive or impossible
3. Consider how experts from completely different fields might approach this problem
4. Identify and challenge hidden assumptions in the problem statement

Track uniqueness: {str(uuid.uuid4())[:8]}
"""
        return enhanced_prompt
        
    def _detect_domains(self, query: str) -> List[str]:
        """Detect relevant domains based on query text."""
        query = query.lower()
        domains = []
        
        # Simple keyword matching for domain detection
        domain_keywords = {
            "science": ["science", "research", "experiment", "hypothesis", "theory", "physics", 
                      "chemistry", "biology", "mathematics", "scientific"],
            "engineering": ["engineering", "build", "design", "construct", "system", "technical",
                          "implementation", "architecture", "optimize", "efficiency", "software", "hardware"],
            "business": ["business", "market", "customer", "product", "service", "strategy",
                        "value", "profit", "company", "startup", "enterprise", "economic"],
            "arts": ["creative", "art", "design", "aesthetic", "visual", "music", "literature",
                    "expression", "emotional", "artistic", "imaginative", "novel"]
        }
    ...
    def _get_domain_techniques(self, domains: List[str]) -> List[str]:
        """Get creativity techniques for the given domains."""
        techniques = []
        
        # Get unique techniques from all relevant domains
        for domain in domains:
            if domain in self.domain_knowledge:
                techniques.extend(self.domain_knowledge[domain])
                
        # Ensure uniqueness
        techniques = list(set(techniques))
        
        # Select a random subset to avoid overwhelming
        if len(techniques) > 5:
            techniques = random.sample(techniques, 5)
            
        return techniques
        
    def generate_novel_perspective(self, query: str, existing_perspectives: List[Dict]) -> Dict:
        """Generate a novel perspective that hasn't been considered yet."""
        # Extract existing perspective names
        existing_names = {p.get("name", "") for p in existing_perspectives}
        
        # Define perspective frameworks that might not have been used
        potential_frameworks = [
            ...
            "Extreme User Perspective",
            "Adjacent Industry Analysis",
            "Temporal Shift Analysis",
            "Inverted Problem Framing",
            "Zero-to-One Framework",
            "Cross-Disciplinary Approach",
            "Scale Shifting Perspective"
        ]
        
        # Filter out already used perspectives
        novel_frameworks = [f for f in potential_frameworks if f not in existing_names]
        
        if not novel_frameworks:
            # If all frameworks used, create a hybrid perspective
            selected_pair = random.sample(list(existing_names), 2) if len(existing_names) > 1 else [list(existing_names)[0], "Novel Approach"]
            framework = f"Hybrid: {selected_pair[0]} + {selected_pair[1]}"
        else:
            # Select a novel framework
            framework = random.choice(novel_frameworks)
            
        # Create a new perspective based on the selected framework
        return {
            "name": framework,
            "description": f"A novel perspective using the {framework} approach",
            "insights": f"Applying {framework} reveals new possibilities in the problem space",
            "limitations": "To be determined through application and feedback"
        }
        
    def track_solution(self, solution: str):
        """Track solutions to avoid repetition."""
        # Create a simplified hash of the solution to track uniqueness
        solution_hash = hash(solution[:200])
        self.previous_approaches.add(solution_hash)
        
    def is_novel_solution(self, solution: str) -> bool:
        """Check if a solution is novel compared to previous solutions."""
        solution_hash = hash(solution[:200])
        return solution_hash not in self.previous_approaches


class AdaptiveWorkflow:
    """Module to enhance the adaptability of the multi-agent system."""
    
    def __init__(self, executive_agent):
        self.executive = executive_agent
        self.pattern_memory = {}
        self.workflow_templates = {
            "standard": {
                "analysis": {"agent": "analyst", "next": ["creative", "logical"]},
                "creative": {"agent": "creative", "next": ["synthesis"]},
                "logical": {"agent": "logical", "next": ["synthesis"]},
                "synthesis": {"agent": "synthesizer", "next": ["critic"]},
                "critic": {"agent": "critic", "next": ["implementation"]},
                "implementation": {"agent": "implementer", "next": ["evaluation"]},
                "evaluation": {"agent": "evaluator", "next": ["final"]}
            },
            "creative_focus": {
                "analysis": {"agent": "analyst", "next": ["creative", "creative_alt"]},
                "creative": {"agent": "creative", "next": ["logical"]},
                "creative_alt": {"agent": "creative", "next": ["logical"], "params": {"alternative": True}},
                "logical": {"agent": "logical", "next": ["synthesis"]},
                "synthesis": {"agent": "synthesizer", "next": ["critic"]},
                "critic": {"agent": "critic", "next": ["creative_ref"]},
                "creative_ref": {"agent": "creative", "next": ["implementation"], "params": {"refine": True}},
                "implementation": {"agent": "implementer", "next": ["evaluation"]},
                "evaluation": {"agent": "evaluator", "next": ["final"]}
            },
            "technical_focus": {
                "analysis": {"agent": "analyst", "next": ["logical"]},
                "logical": {"agent": "logical", "next": ["creative"]},
                "creative": {"agent": "creative", "next": ["synthesis"]},
                "synthesis": {"agent": "synthesizer", "next": ["critic"]},
                "critic": {"agent": "critic", "next": ["implementation"]},
                "implementation": {"agent": "implementer", "next": ["implementation_rev"]},
                "implementation_rev": {"agent": "implementer", "next": ["evaluation"], "params": {"revise": True}},
                "evaluation": {"agent": "evaluator", "next": ["final"]}
            },
            "iterative_improvement": {
                ...
                "logical": {"agent": "logical", "next": ["synthesis"]},
                "synthesis": {"agent": "synthesizer", "next": ["critic"]},
                "critic": {"agent": "critic", "next": ["synthesis_ref"]},
                "synthesis_ref": {"agent": "synthesizer", "next": ["implementation"], "params": {"refine": True}},
                "implementation": {"agent": "implementer", "next": ["critic_impl"]},
                "critic_impl": {"agent": "critic", "next": ["implementation_ref"], "params": {"focus": "implementation"}},
                "implementation_ref": {"agent": "implementer", "next": ["evaluation"], "params": {"refine": True}},
                "evaluation": {"agent": "evaluator", "next": ["final"]}
            }
        }
    
    def detect_optimal_workflow(self, query: str) -> str:
        """Determine the most suitable workflow template for a given query."""
        query = query.lower()
        
        # Technical/implementation focused
        technical_keywords = ["implement", "code", "algorithm", "technical", "efficiency", 
                            "optimize", "performance", "software", "hardware", "system"]
        
        # Creative/innovative focused
        creative_keywords = ["creative", "novel", "innovative", "unique", "original", 
                           "imagine", "design", "new approach", "brainstorm", "conceptual"]
        
        # Iterative refinement focused
        iterative_keywords = ["improve", "refine", "iterate", "enhance", "perfect", 
                            "revise", "polish", "optimize", "better version", "next generation"]
        
        # Count keyword matches for each workflow type
        technical_score = sum(1 for keyword in technical_keywords if keyword in query)
        creative_score = sum(1 for keyword in creative_keywords if keyword in query)
        iterative_score = sum(1 for keyword in iterative_keywords if keyword in query)
        
        # Determine the highest scoring workflow
        if technical_score > creative_score and technical_score > iterative_score:
            return "technical_focus"
        elif creative_score > technical_score and creative_score > iterative_score:
            return "creative_focus"
        elif iterative_score > technical_score and iterative_score > creative_score:
            return "iterative_improvement"
        else:
            return "standard"
    
    def customize_workflow(self, template_name: str, query: str, agents_dict: Dict) -> Dict:
        """Customize a workflow template for the specific query and available agents."""
        # Get the template
        template = self.workflow_templates.get(template_name, self.workflow_templates["standard"])
        
        # Create a customized workflow with actual agent references
        workflow = {}
        for stage, config in template.items():
            agent_name = config["agent"]
            
            # Set up agent reference
            agent_reference = None
            if hasattr(agents_dict, agent_name):
                agent_reference = getattr(agents_dict, agent_name)
            elif agent_name in agents_dict:
                agent_reference = agents_dict[agent_name]
            else:
                # Fall back to a standard agent if the specified one isn't available
                fallback_map = {
                    "creative_alt": "creative",
                    "creative_ref": "creative",
                    "implementation_rev": "implementer",
                    "synthesis_ref": "synthesizer",
                    "critic_impl": "critic"
                }
                fallback = fallback_map.get(agent_name)
                if fallback and hasattr(agents_dict, fallback):
                    agent_reference = getattr(agents_dict, fallback)
                elif fallback and fallback in agents_dict:
                    agent_reference = agents_dict[fallback]
        ...
        # Make sure the workflow is connected
        self._verify_workflow_connectivity(workflow)
        
        return workflow
    
    def _verify_workflow_connectivity(self, workflow: Dict) -> None:
        """Ensure that the workflow is properly connected with no broken references."""
        # Get all stage names
        all_stages = set(workflow.keys())
        all_stages.add("final")  # Add the implicit final stage
        
        # Check that all "next" references are valid
        for stage, config in workflow.items():
            for next_stage in config["next"]:
                if next_stage not in all_stages:
                    # Fix by pointing to the final stage
                    config["next"].remove(next_stage)
                    config["next"].append("final")
        
        # Ensure there's at least one entry point
        if "analysis" not in workflow:
            # Create a basic analysis stage as an entry point
            workflow["analysis"] = {
                "agent": workflow[list(workflow.keys())[0]]["agent"],  # Use the first available agent
                "next": [list(workflow.keys())[1]] if len(workflow) > 1 else ["final"],
                "params": {}
            }
    
    def apply_dynamic_adjustments(self, workflow: Dict, agent_performances: Dict) -> Dict:
        """Apply dynamic adjustments to the workflow based on agent performance."""
        adjusted_workflow = workflow.copy()
        
        # Identify underperforming agents
        underperforming = [agent for agent, perf in agent_performances.items() if perf < 0.6]
        
        for agent in underperforming:
            # Look for this agent in the workflow
            for stage, config in workflow.items():
                if config["agent"].name.lower() == agent.lower():
                    # Add a parallel agent to complement the underperforming one
                    if agent.lower() == "creative":
                        # Add logical to support creative
                        logical_stage = f"{stage}_logical"
                        if "logical" in [a.name.lower() for a in adjusted_workflow.values()]:
                            logical_agent = [a for s, a in adjusted_workflow.items() 
                                         if a["agent"].name.lower() == "logical"][0]["agent"]
                            
                            # Insert logical thinking in parallel
                            adjusted_workflow[logical_stage] = {
                                "agent": logical_agent,
                                "next": config["next"],
                                "params": {"support": agent}
                            }
                            
                            # Update the original stage to include the new branch
                            # Adjust the incoming connections to include the new stage
                            for s, c in adjusted_workflow.items():
                                if stage in c["next"]:
                                    c["next"].append(logical_stage)
                    
                    elif agent.lower() == "logical":
                        # Add creative to support logical
                        creative_stage = f"{stage}_creative"
                        if "creative" in [a.name.lower() for a in adjusted_workflow.values()]:
                            creative_agent = [a for s, a in adjusted_workflow.items() 
                                          if a["agent"].name.lower() == "creative"][0]["agent"]
                            
                            # Insert creative thinking in parallel
                            adjusted_workflow[creative_stage] = {
                                "agent": creative_agent,
                                "next": config["next"],
                                "params": {"support": agent}
                            }
                            
                            # Adjust the incoming connections
                            for s, c in adjusted_workflow.items():
                                if stage in c["next"]:
                                    c["next"].append(creative_stage)
        
        return adjusted_workflow


class InterAgentFeedbackChannel:
    """Enables more sophisticated feedback loops between agents."""
    
    def __init__(self, neocortex):
        self.neocortex = neocortex
        self.feedback_history = {}
        
    def register_feedback_channel(self, from_agent: str, to_agent: str, feedback_type: str):
        """Register a feedback channel between two agents."""
        channel_id = f"{from_agent}_to_{to_agent}_{feedback_type}"
        self.feedback_history[channel_id] = []
        
    def send_feedback(self, from_agent: str, to_agent: str, feedback_type: str, content: Any):
        ...
        # Store in history
        if channel_id not in self.feedback_history:
            self.feedback_history[channel_id] = []
        self.feedback_history[channel_id].append(feedback)
        
        # Send the feedback
        self._deliver_feedback(feedback)
        
    def get_feedback_history(self, agent_name: str = None, feedback_type: str = None) -> List[Dict]:
        """Get feedback history, optionally filtered by agent name or feedback type."""
        result = []
        
        for channel_id, feedback_list in self.feedback_history.items():
            for feedback in feedback_list:
                # Apply filters
                if agent_name and (feedback["from"] != agent_name and feedback["to"] != agent_name):
                    continue
    ...
    def get_latest_feedback(self, to_agent: str, feedback_type: str = None) -> Optional[Dict]:
        """Get the latest feedback sent to a specific agent, optionally filtered by type."""
        relevant_feedback = []
        ...
        for channel_id, feedback_list in self.feedback_history.items():
            ...
            # Check if this channel targets the requested agent
            if f"_to_{to_agent}_" not in channel_id:
                continue
                
            # Apply type filter if specified
            if feedback_type and feedback_type not in channel_id:
                continue
                
            # Get the latest feedback from this channel
            latest = feedback_list[-1]
            relevant_feedback.append(latest)
                
        if not relevant_feedback:
            return None
            
        # Return the most recent feedback across all relevant channels
        return sorted(relevant_feedback, key=lambda x: x["timestamp"])[-1]
        
    def _deliver_feedback(self, feedback: Dict):
        """Deliver feedback to the target agent."""
        # Find the target agent
        to_agent = feedback["to"]
        from_agent = feedback["from"]
        
        # Get agent references from the neocortex
        target_agent = None
        source_agent = None
        
        if hasattr(self.neocortex, "multi_agent_system"):
            mas = self.neocortex.multi_agent_system
            if hasattr(mas, "executive"):
                executive = mas.executive
                
                # Try to find agents in the executive's workflow
                if hasattr(executive, "workflow"):
                    for stage_config in executive.workflow.values():
                        agent = stage_config.get("agent")
                        if agent and agent.name.lower() == to_agent.lower():
                            target_agent = agent
                        if agent and agent.name.lower() == from_agent.lower():
                            source_agent = agent
        
        # If we found both agents, deliver the feedback
...
class DynamicPromptEnhancer:
    """Enhances agent prompts based on context and feedback."""
    
    def __init__(self):
        self.prompt_templates = {}
        self.prompt_history = {}
        
    def register_template(self, agent_type: str, stage: str, template: str):
        """Register a prompt template for a specific agent type and stage."""
        key = f"{agent_type}_{stage}"
        self.prompt_templates[key] = template
        
    def create_enhanced_prompt(self, agent_type: str, stage: str, base_prompt: str, 
                              context: Dict, feedback: List[Dict] = None) -> str:
        """Create an enhanced prompt based on templates, context and feedback."""
        # Try to get a specific template
        key = f"{agent_type}_{stage}"
        template = self.prompt_templates.get(key, base_prompt)
        
        # Store relevant portions of context for template formatting
        format_vars = {
        ...
        if feedback:
            ...
            format_vars["feedback"] = feedback_summary
        else:
            format_vars["feedback"] = ""
            
        # Add special instructions based on stage params
        params = context.get("params", {})
        special_instructions = ""
        
        if params.get("refine"):
            special_instructions += """
            This is a refinement stage. You should:
            1. Focus on improving the existing solution rather than creating a new one
            2. Address specific feedback and criticisms
            3. Maintain the core approach while enhancing its implementation
            """
            
        if params.get("alternative"):
            special_instructions += """
            This is an alternative exploration stage. You should:
            1. Deliberately take a different approach than what might be obvious
            2. Consider perspectives from unrelated domains
            3. Challenge core assumptions in the problem formulation
            """
            
        if params.get("support"):
            special_instructions += f"""
            This is a support stage for the {params.get('support')} agent. You should:
            1. Complement the {params.get('support')} agent's approach
            2. Provide insights that the {params.get('support')} agent might miss
            3. Focus on the strengths of your reasoning style while acknowledging the other agent's contribution
            """
            
        format_vars["special_instructions"] = special_instructions
        
        # Apply template formatting
        try:
            enhanced_prompt = template.format(**format_vars)
        except KeyError:
            # Fall back to base prompt if formatting fails
            enhanced_prompt = base_prompt
            
        # Store in history
        if key not in self.prompt_history:
            self.prompt_history[key] = []
            
        self.prompt_history[key].append({
            "timestamp": datetime.now().isoformat(),
    ...
    def _summarize_feedback(self, feedback: List[Dict]) -> str:
        """Summarize feedback for inclusion in prompts."""
        if not feedback:
            return ""
            
        summary = "FEEDBACK FROM OTHER AGENTS:\n"
        
        for fb in feedback:
            from_agent = fb.get("from", "Unknown")
            ...
            summary += f"\nFrom {from_agent} ({feedback_type}):\n{content_str}\n"
            
        return summary


def install_enhancements(neocortex):
    """Install the enhancements to the existing NeoCortex and multi-agent system."""
    try:
        # Only proceed if multi-agent system exists
        if not hasattr(neocortex, "multi_agent_system"):
            print("Multi-agent system not found. Enhancements cannot be installed.")
            return False
            
        mas = neocortex.multi_agent_system
        
        # Check if executive agent exists
        if not hasattr(mas, "executive"):
            print("Executive agent not found. Enhancements cannot be installed.")
            return False
            
        executive = mas.executive
        
        # Create enhancement modules
        creative_enhancer = CreativeEnhancer(neocortex)
        adaptive_workflow = AdaptiveWorkflow(executive)
        feedback_channel = InterAgentFeedbackChannel(neocortex)
        prompt_enhancer = DynamicPromptEnhancer()
        
        # Store the enhancers on the multi-agent system
        mas.creative_enhancer = creative_enhancer
        mas.adaptive_workflow = adaptive_workflow
        mas.feedback_channel = feedback_channel
        mas.prompt_enhancer = prompt_enhancer
        
        # Extend the process_query method of multi-agent system
        original_process_query = mas.process_query
        
        def enhanced_process_query(query):
            """Enhanced version of process_query with adaptive workflows and creativity."""
            # Detect optimal workflow template
            workflow_template = adaptive_workflow.detect_optimal_workflow(query)
            
            # Customize workflow for this query
            if hasattr(executive, "analyst") and hasattr(executive, "creative") and hasattr(executive, "logical"):
                agents_dict = {
                    "analyst": executive.analyst,
                    "creative": executive.creative,
                    "logical": executive.logical,
                    "synthesizer": executive.synthesizer,
                    "critic": executive.critic,
                    "implementer": executive.implementer,
                    "evaluator": executive.evaluator
                }
                
                # Create a custom workflow
                custom_workflow = adaptive_workflow.customize_workflow(workflow_template, query, agents_dict)
                
                # Store the original workflow
                original_workflow = executive.workflow
                
                # Apply the custom workflow
                executive.workflow = custom_workflow
                
                # Register feedback channels
                for stage, config in custom_workflow.items():
                    agent = config["agent"]
                    for next_stage in config["next"]:
                        if next_stage in custom_workflow:
                            next_agent = custom_workflow[next_stage]["agent"]
                            feedback_channel.register_feedback_channel(
                                agent.name,
                                next_agent.name,
                                "workflow_feedback"
                            )
                ...
                executive.workflow = original_workflow
                
                return result
            else:
                # Fall back to original processing if agent structure doesn't match expectations
                return original_process_query(query)
        
        # Replace the method
        mas.process_query = enhanced_process_query
        
        # Enhance the CreativeAgent's _process_implementation method
        if hasattr(executive, "creative"):
            original_creative_process = executive.creative._process_implementation
            
            def enhanced_creative_process(input_data: Dict) -> Dict:
                """Enhanced version of the creative agent's processing with domain-aware creativity."""
                query = input_data.get("query", "")
                analysis = input_data.get("analysis", "")
                
                # Get any criticism feedback
                criticism = None
                latest_feedback = feedback_channel.get_latest_feedback(
                    executive.creative.name,
                    "feedback"
                )
                if latest_feedback:
                    criticism = str(latest_feedback.get("content", ""))
                
                # Generate enhanced system prompt
                enhanced_system_prompt = creative_enhancer.enhance_creativity_prompt(
                    query, 
                    input_data.get("previous_output", ""),
                    criticism
                )
                
                # Add the enhanced prompt to input data
                input_data["enhanced_system_prompt"] = enhanced_system_prompt
                
                # Process with original method
                result = original_creative_process(input_data)
                
                # Track this solution
                if "creative_output" in result:
                    creative_enhancer.track_solution(result["creative_output"])
                
                # Check if we need to add a novel perspective
                if "approaches" in result and len(result["approaches"]) < 3:
                    # Generate a novel approach
                    novel_approach = creative_enhancer.generate_novel_perspective(
                        query,
                        result.get("approaches", [])
                    )
                    result["approaches"].append(novel_approach)
                
                return result
            
            # Replace the method
            executive.creative._process_implementation = enhanced_creative_process
        
        # Add bidirectional feedback capability
        def enhanced_implementation(self, input_data: Dict) -> Dict:
            """Enhanced implementation with bidirectional feedback."""
            # Process normally
            result = self._process_implementation(input_data)
            
            # Send feedback to creative and logical agents about implementation challenges
            if hasattr(executive, "creative") and hasattr(executive, "logical") and hasattr(self, "name"):
                if self.name == "Implementer" and "code_blocks" in result:
                    # Extract implementation challenges
                    challenges = {
                        "implementation_challenges": "Identified implementation considerations that could benefit from creative solutions",
                        "code_size": len(str(result.get("code_blocks", []))),
                        "suggested_refinements": "Consider alternative approaches for challenging aspects"
                    }
                    
                    # Send feedback to creative agent
                    feedback_channel.send_feedback(
                        self.name,
                        "Creative",
                    ...
                    # Send feedback to logical agent
                    feedback_channel.send_feedback(
                        self.name,
                        "Logical",
                        "implementation_feedback",
        ...
        # Apply enhanced implementation to implementation agent
        if hasattr(executive, "implementer"):
            executive.implementer._enhanced_implementation = enhanced_implementation
            
            # Store original method
            original_implementer_process = executive.implementer.process
            
            # Replace process method to use enhanced implementation
            def enhanced_implementer_process(input_data: Dict) -> Dict:
                self = executive.implementer  # 'self' reference for the agent
                ...
                try:
                    ...
                    result = self._enhanced_implementation(input_data)
            ...
            # Replace the method
            executive.implementer.process = enhanced_implementer_process
        
        # Add similar enhancements to the critic agent
        if hasattr(executive, "critic"):
            original_critic_process = executive.critic._process_implementation
            
            def enhanced_critic_process(input_data: Dict) -> Dict:
                """Enhanced version of the critic process with feedback loops."""
                result = original_critic_process(input_data)
                
                # Send feedback to previous stages
                if "criticism" in result and "weaknesses" in result:
                    feedback_content = {
                        "criticism_summary": result["criticism"][:200] + "..." if len(result["criticism"]) > 200 else result["criticism"],
                    ...
                    # Send to synthesizer
                    if hasattr(executive, "synthesizer"):
                        feedback_channel.send_feedback(
                            "Critic",
                            "Synthesizer",
                            "feedback",
            ...
            # Replace the method
            executive.critic._process_implementation = enhanced_critic_process
            
        print("Enhancements successfully installed!")
        return True
    
    except Exception as e:
        print(f"Error installing enhancements: {str(e)}")
        return False


# Additional utility for custom prompt template registration
def register_prompt_templates(neocortex):
    """Register enhanced prompt templates for different agents."""
    if not hasattr(neocortex, "multi_agent_system") or not hasattr(neocortex.multi_agent_system, "prompt_enhancer"):
        print("Prompt enhancer not found. Templates cannot be registered.")
        return False
    
    prompt_enhancer = neocortex.multi_agent_system.prompt_enhancer
    
    # Creative agent templates
    creative_template = """You are a creative agent specialized in generating innovative approaches to problems.
Your task is to:
1. Think beyond conventional solutions and constraints
2. Generate multiple diverse and novel approaches
3. Consider interdisciplinary perspectives and analogies
...
Provide multiple creative approaches that open new possibilities for solving the problem.
"""
    prompt_enhancer.register_template("creative", "standard", creative_template)
    
    # Logical agent templates
    logical_template = """You are a logical reasoning agent specialized in structured problem-solving.
Your task is to:
1. Apply deductive and inductive reasoning techniques
2. Develop clear, step-by-step logical frameworks for solving problems
3. Identify and validate assumptions
...
Provide a rigorous logical analysis that leads to a well-structured solution approach.
"""
    prompt_enhancer.register_template("logical", "standard", logical_template)
    
    # Synthesizer agent templates
    synthesizer_template = """You are a synthesis agent specialized in integrating multiple perspectives and approaches.
Your task is to:
1. Identify complementary elements across different solutions and perspectives
2. Resolve contradictions and tensions between approaches
3. Create a unified framework that leverages the strengths of each approach
...
Provide a synthesis that integrates diverse perspectives into a cohesive solution.
"""
    prompt_enhancer.register_template("synthesizer", "standard", synthesizer_template)
    
    # Additional template for creative refinement
    creative_refinement_template = """You are a creative agent tasked with refining an existing solution.
Your task is to:
1. Enhance the originality of the existing solution without completely changing it
2. Address specific weaknesses identified in feedback
3. Introduce novel elements that complement the current approach
4. Balance innovation with maintaining continuity with previous work
5. Focus on addressing implementation challenges with creative workarounds
...
Provide refined creative approaches that strengthen the existing solution while addressing feedback.
"""
    prompt_enhancer.register_template("creative", "refinement", creative_refinement_template)
    
    print("Prompt templates successfully registered!")
    return True
